...
public class SistemaViajes {
	...
	public boolean agregarTarjetaJubilacion(String codigo, double porcentajeDeDescuento, boolean descuentoActivo) throws Exception {
		...
		return tarjetas.add(new TarjetaJubilacion(codigo, calcularProximoIDTarjeta(), descuentoActivo, porcentajeDeDescuento));
	}
	...
	public List<Tarjeta> traerTarjetaJubilacionActivaConPorcentajeMayorA(double porcentajeMayorA){
		List<Tarjeta> lstAux = new ArrayList<Tarjeta>();
		
		for (Tarjeta tarjeta : tarjetas) {
			if(tarjeta instanceof TarjetaJubilacion) {
				TarjetaJubilacion aux = (TarjetaJubilacion) tarjeta;
				if(aux.isDescuentoActivo() && aux.getPorcentajeDeDescuento() > porcentajeMayorA) {
					lstAux.add(tarjeta);
				}
